...
SELECT
  ...
  lp.pago_id, lp.metodo, lp.estado AS estado_pago, lp.detalle
FROM reservas r
JOIN canchas c
...
if ($hora_desde && $hora_hasta) {
  $sql .= " AND NOT (r.hora_fin <= ? OR r.hora_inicio >= ?) ";
  $params[] = $hora_desde;
  $params[] = $hora_hasta;
  $types .= "ss";
} elseif ($hora_desde) {
...
function cleanDetalle(?string $d): string
{
  $d = trim((string)$d);
  if ($d === '') return '';
  $d = preg_replace('/\s*,\s*/', ', ', $d); // normaliza comas
  $d = preg_replace('/\s+/', ' ', $d);      // colapsa espacios
  return trim($d);
}
?>
<main>
  ...
  <style>
    ...
    .row-highlight {
      animation: hiBlink 1.2s ease-in-out 3;
    }
    ...
    /* Para que el iconito quede “botón-like” y alineado con acciones */
    .btn-icon {
      display: inline-flex;
      align-items: center;
      justify-content: center;
  ...
  <div class="section">
    ...
    <table>
      ...
      <?php if ($reservas->num_rows): while ($r = $reservas->fetch_assoc()):
          ...
          $detalleSplit = cleanDetalle($r['detalle'] ?? null);
          $hasSplit = ($detalleSplit !== '');
      ?>
            ...
            <td>
              <div class="action-buttons">
                <?php if (!$isPagado && !empty($r['pago_id'])): ?>
                  ...
                  <?php if ($hasSplit): ?>
                    <span class="tip btn-icon" data-tip="Pago dividido entre: <?= htmlspecialchars($detalleSplit) ?> y el titular de la reserva" aria-label="Pago dividido">👥</span>
                  <?php endif; ?>
                ...
                <?php elseif ($isPagado): ?>
                  ...
                  <?php if ($hasSplit): ?>
                    <span class="tip btn-icon" data-tip="Pago dividido entre: <?= htmlspecialchars($detalleSplit) ?>" aria-label="Pago dividido">👥</span>
                  <?php endif; ?>
        ...
        <tr>
          <td colspan="9" style="text-align:center;">Sin reservas para los filtros seleccionados.</td>
        </tr>
      <?php endif; ?>
    </table>
  </div>
