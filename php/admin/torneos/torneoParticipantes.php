<?php
/**************************************************************
 * file: admin/torneos/torneoParticipantes.php
 * SOLO LECTURA — Bracket SVG (diseño: más espacio en R1 y textos por ronda)
 *************************************************************/
...
/* Slots -> potencia de 2 */
$cap = max(2, (int)$torneo['capacidad']);
function nextPow2($n){ $p=1; while($p<$n) $p<<=1; return $p; }
$slots = nextPow2(max(count($participantes), 2, $cap));
  ...
  <div class="section-header" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
    <div style="display:flex;flex-direction:column;gap:6px;">
      ...
      <div class="meta-chips">
        <span class="chip chip-brand"><?= htmlspecialchars($torneo['club'] ?: '—') ?></span>
...
(function(){
  ...
  const V_GAP = 30, V_GAP_R1 = 10; // por qué: más separación SOLO en Ronda 1
  ...
  const STEP0   = (BOX_H*2 + V_GAP); // el paso vertical base NO cambia

  const centerY = (r,i)=> M_TOP + (i + 0.5) * (STEP0 * Math.pow(2, r));
  const pairGap = (r)=> r===0 ? V_GAP_R1 : V_GAP; // gap interno por ronda
  ...
  function drawBox(x,y,w,h,empty,text){
    ...
    if(empty) r.setAttribute('stroke-dasharray','4 3'); // por qué: marcar placeholders
  ...
  const labelForRound = (r,total)=>{
    if (r===0) return null;                 // usa nombres o "Sin participante"
    if (r===total-1) return 'Finalista';
    if (r===total-2) return 'Semifinalista';
    return 'Clasificado';
  };
  const CHAMP_LABEL = 'Ganador';

  /* Tamaño (+1 columna Campeón) */
