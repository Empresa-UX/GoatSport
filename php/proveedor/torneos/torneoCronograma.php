<?php
/* =========================================================================
 * PROVEEDOR — Bracket de torneo (SOLO LECTURA, estilo ADMIN)
 * Ruta sugerida: /proveedor/torneoParticipantes.php
 * ========================================================================= */
...
/* Torneo (validar que pertenece al proveedor) */
$ts = $conn->prepare("
  SELECT t.torneo_id, t.nombre, t.tipo, t.capacidad,
...
/* Partidos del torneo (incluye nombres y wiring de siguiente partido) */
$ms = $conn->prepare("
  SELECT
...
/* Fallback: participantes aceptados si no hay partidos */
$participantes = [];
if (!$matchesRows) {
  $ps = $conn->prepare("
...
/* Propagar ganadores hacia adelante */
if ($byId) {
  ...
  foreach ($matchesRows as $m) {
    ...
    // No sobreescribir si ya hay alguien
    if ($pos === 'j1' && $byId[$nextId]['jugador1_id'] === null) {
      $byId[$nextId]['jugador1_id'] = $gan;
      $byId[$nextId]['j1_nombre']   = $byId[$pid]['ganador_nombre'] ?: ('#'.$gan);
    } elseif ($pos === 'j2' && $byId[$nextId]['jugador2_id'] === null) {
...
<div class="section">
  ...
  <style>
    :root{ --brand:#0f766e; }
    .btn-action{
      cursor:pointer;display:inline-flex;align-items:center;gap:8px;padding:8px 12px;
      text-decoration:none;font-weight:800;font-size:14px;white-space:nowrap;border-radius:10px;border:1px solid #bfd7ff;background:#e0ecff;color:#1e40af
    }
...
(function(){
  ...
  // Sin partidos → render semilla de participantes
  if(!rounds.length){
