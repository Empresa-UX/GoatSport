...
// Basado en recepcionista/notificacionesAction.php, adaptado a rol admin
...
function typesFor(array $uids): string {
    return str_repeat('i', count($uids));
}

$action = $_POST['action'] ?? '';

/* mark_all_read: para todas las del admin */
if ($action === 'mark_all_read') {
    $ph = placeholders(count($uids));
    $sql = "
        UPDATE notificaciones 
        SET leida = 1 
        WHERE usuario_id IN ($ph) 
          AND leida = 0
    ";
...
/* toggle_read (compatibilidad legacy) */
...
/* Compat legacy: mark_read / mark_unread */
