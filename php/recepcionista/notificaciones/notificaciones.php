<?php
/* =========================================================================
 * file: php/recepcionista/notificaciones/notificaciones.php  (REEMPLAZA)
 * ========================================================================= */
...
$tipo   = $_GET['tipo']   ?? '';
$origen = SHOW_ORIGEN ? ($_GET['origen'] ?? '') : '';
...
if ($estado === 'no_leidas') { $where .= " AND leida = 0 "; }
elseif ($estado === 'leidas'){ $where .= " AND leida = 1 "; }

if (SHOW_ORIGEN && $origen !== '') { $where .= " AND origen = ? "; $params[] = $origen; $types .= 's'; }
if ($tipo   !== '') { $where .= " AND tipo   = ? "; $params[] = $tipo;   $types .= 's'; }

/* Conteo no leídas (badge global) */
$sqlCountUnread = "SELECT COUNT(*) AS cant FROM notificaciones WHERE usuario_id IN ($placeholders) AND leida = 0";
...
/* Tipos disponibles */
$sqlTipos = "SELECT DISTINCT tipo FROM notificaciones WHERE usuario_id IN ($placeholders) ORDER BY tipo ASC";
...
/* Listado (paginado) */
$sqlList = "SELECT notificacion_id, usuario_id, tipo, origen, titulo, mensaje, creada_en, leida
            FROM notificaciones
            $where
...
function tipoToLabel(string $tipo): string {
    static $map = [
        ...
        'pago_confirmado'    => 'Pago club confirmado',
    ];
...
<main>
  <div class="section">
    <div class="section-header" style="align-items:center; gap:12px; flex-wrap:wrap;">
      <h2 style="margin-right:auto; display:flex; align-items:center; gap:10px;">Notificaciones
    ...
    <form method="GET" action="notificaciones.php" class="filterbar" id="filtros">
      ...
      <div class="f-field">
        ...
        <select name="origen" class="f-select">
          <option value="" <?= $origen===''?'selected':'' ?>>Todos</option>
          <?php foreach (['admin','proveedor','recepcion','sistema','app'] as $opt): ?>
            <option value="<?= $opt ?>" <?= $origen===$opt?'selected':'' ?>><?= ucfirst($opt) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <?php endif; ?>
      <div class="f-field">
        ...
        <select name="tipo" class="f-select">
          ...
          <?php foreach ($tiposDisponibles as $t): ?>
            <option value="<?= htmlspecialchars($t) ?>" <?= $tipo===$t?'selected':'' ?>><?= htmlspecialchars(tipoToLabel($t)) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    ...
    <style>
      ...
      .f-select{width:100%; padding:10px 12px; border:1px solid #d6dadd; border-radius:10px; background:#fff;
      ...
      .badge-count{display:inline-flex;align-items:center;gap:8px;padding:4px 12px;border-radius:999px;
      ...
      .notif-tipo-pill{display:inline-block;padding:2px 10px;border-radius:999px;font-size:11px;text-transform:uppercase;letter-spacing:.4px}
      ...
      .chip{padding:2px 10px;border-radius:999px;font-size:12px;border:1px solid transparent;min-width:100px;text-align:center}
      ...
      .btn-load{background:#e9eef1;color:#1f2937;border:0;border-radius:10px;padding:9px 14px;cursor:pointer;font-weight:600}
      .btn-load:disabled{opacity:.6;cursor:default}
    </style>

    <!-- Contenedor robusto: tabla + botón -->
    <div id="noti-list">
      <table>
        <thead>
          <tr>
        ...
        <?php if ($notis->num_rows): while($n=$notis->fetch_assoc()):
          ...
          $dest = ((int)$n['usuario_id'] === $recepcionista_id) ? 'Recepcionista' : 'Proveedor';
          $labelTipo = tipoToLabel((string)$n['tipo']);

          // >>> NUEVO: formatos requeridos
        ...
        ?>
          <tr class="<?= $rowClass ?>" data-nid="<?= (int)$n['notificacion_id'] ?>">
            ...
            <td class="col-tipo"><span class="<?= $pillClass ?>"><?= htmlspecialchars($labelTipo) ?></span></td>
            <?php if (SHOW_ORIGEN): ?><td><?= htmlspecialchars(ucfirst($n['origen'])) ?></td><?php endif; ?>
            <?php if (SHOW_DEST):   ?><td><?= htmlspecialchars($dest) ?></td><?php endif; ?>
            <td class="state"><span class="chip <?= $estadoCls ?>"><?= $estadoTxt ?></span></td>
            <td class="action">
              <?php if (!$isLeida): ?>
                <form class="form-mark-read" method="POST" action="notificacionesAction.php" style="display:inline-block;">
                  <input type="hidden" name="csrf" value="<?= htmlspecialchars($csrf) ?>">
                  <input type="hidden" name="action" value="mark_read"><!-- sólo leer; no reversible -->
        ...
        <?php endwhile; else: ?>
          <tr><td colspan="<?= 7 + (int)SHOW_ORIGEN + (int)SHOW_DEST ?>" style="text-align:center;">No tienes notificaciones.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
      ...
      <?php if ($hasMore): ?>
        <div class="load-more-wrap">
          <button id="load-more" class="btn-load" data-next="<?= (int)($page+1) ?>" data-limit="<?= (int)$limit ?>">Cargar más</button>
        </div>
      <?php endif; ?>
    </div>
  </div>
...
<script>
/* ---------- Filtros instantáneos (reemplaza #noti-list completo) ---------- */
(function(){
...
/* ---------- Marcar leído (una vez); si filtro=no_leidas, quitar fila ---------- */
function wireMarkRead(){
  ...
  document.querySelectorAll('.form-mark-read').forEach(f => {
    ...
    f.addEventListener('submit', async (ev)=>{
      ev.preventDefault();
      const btn = f.querySelector('button.toggle-read'); btn.disabled = true;
      try{
        ...
        if (currentEstado === 'no_leidas') {
          ...
          chip.classList.remove('chip-warn'); chip.classList.add('chip-ok'); chip.textContent='Leído';
          row.querySelector('td.action').innerHTML = '<span class="icon-read" title="Leída">✅</span>';
        }

        const badge = document.getElementById('badge-no-leidas');
        if (badge){
          const strong = badge.querySelector('strong');
          let n = parseInt(strong.textContent||'0',10); n = Math.max(0, n-1);
...
/* ---------- Cargar más (usa #noti-list; estable) ---------- */
  ...
  btn.addEventListener('click', async ()=>{
    ...
    try{
      ...
      if (newBtn) {
        ...
        btn.parentElement.remove(); // no hay más
      }
      wireMarkRead();
    }catch(e){
      btn.disabled = false;
