<?php
/* =========================================================================
 * file: php/recepcionista/notificaciones/notificaciones.php  (REEMPLAZO)
 * Diseño copiado del ADMIN, adaptado a recepcionista:
 * - Misma UI: Título/Mensaje envuelven, pills de tipo y ORIGEN, cuadrito "Visto"
 * - Filtros: estado, tipo, origen
 * - Paginación + “Cargar más”
 * - Marca como leída (no reversible desde UI)
 * - Ámbito: notificaciones dirigidas al recepcionista y/o al proveedor asignado
 * ========================================================================= */
...
/* Usuario recepcionista + proveedor asociado (si existe) */
...
/* Base WHERE: notificaciones dirigidas al recepcionista/proveedor */
...
/* Tipos disponibles (para este ámbito) */
$sqlTipos = "
...
function tipoToLabel(string $tipo): string {
  static $map = [
    ...
    'resultado_nuevo'       => 'Resultado cargado',
    'resultado_editado'     => 'Resultado editado',
    'resultado_eliminado'   => 'Resultado eliminado',
  ];
...
function tipoToClass(string $tipo): string {
  ...
  if (str_starts_with($tipo, 'resultado'))return 'notif-tipo-pill notif-tipo-resultado';
...
<main>
  <div class="section">
    ...
    <style>
      ...
      .notif-tipo-resultado{background:#f0f9ff;color:#0b5394;border-color:#cfe8ff}
      ...
      .read-indicator{
        ...
        cursor:pointer; transition:background .15s, transform .02s, border-color .15s, box-shadow .15s;
      ...
      .read-indicator.is-checked{
        background:#e6f7f4; border-color:#c8efe8; box-shadow:0 0 0 3px rgba(27,171,157,.12);
      }
      .read-indicator.is-checked::after{ content:'✓'; font-size:18px; line-height:1; font-weight:900; }
