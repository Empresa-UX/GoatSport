...
if ($partido_id <= 0) {
  echo "<main><div class='section'><p>Partido inválido.</p></div></main>";
...
<main>
  <style>
    :root{
      --text:#043b3d; --muted:#6b7a80; --border:#d6dadd;
      --primary:#1bab9d; --primary-600:#159788; --ring:rgba(27,171,157,.12);
    }
    .gs-card{
      max-width: 460px;
      margin: 24px auto; background:#fff; border-radius:16px;
      box-shadow: 0 10px 28px rgba(0,0,0,.10); padding: 22px 22px;
      border:1px solid #eef2f3;
    }
    .gs-title{ font-size:18; font-weight:750; color:var(--text); text-align:center; margin:6px 0 4px }
    ...
    .info-grid{
      ...
      grid-template-areas:
        ...
        "cancha jugadores";
      gap:10px 18px;
      border:1px dashed #e7ecef; border-radius:12px; padding:14px 16px; margin-bottom:16px;
    }
    ...
    .i-partido{ grid-area: partido; }
    .i-fecha{ grid-area: fecha; }
    .i-horario{ grid-area: horario; }
    .i-torneo{ grid-area: torneo; }
    .i-cancha{ grid-area: cancha; }
    .i-jugadores{ grid-area: jugadores; }
    ...
    .pill{
      display:inline-flex; align-items:center; gap:8px; padding:10px 14px; border-radius:999px; width: 48%;
      border:1px solid #cfd8dc; background:#fff; cursor:pointer; user-select:none; transition: box-shadow .15s, border-color .15s;
    }
    ...
    .btn{
      ...
      border-radius:10px; padding:12px 16px; font-weight:700; border:1px solid transparent; cursor:pointer;
      transition: background-color .15s, box-shadow .15s, transform .02s; font-size: 16px; text-decoration: none;
    }
    ...
    @media (max-width: 560px){
      .info-grid{ grid-template-columns: 1fr; grid-template-areas:
        "partido" "fecha" "horario" "torneo" "cancha" "jugadores"; }
    }
  </style>

  <div class="gs-card" role="region" aria-labelledby="title">
    <h2 id="title" class="gs-title">Cargar resultado</h2>
    <p class="gs-sub">Revisa la información y completa el resultado del partido</p>

    <div class="info-grid">
      <div class="i-partido">
        <div class="lbl">Partido</div><div class="val">#<?= (int)$match['partido_id'] ?></div>
      </div>
      <div class="i-fecha">
        <div class="lbl">Fecha</div><div class="val"><?= htmlspecialchars($fechaLeg) ?></div>
      </div>
      <div class="i-horario">
        <div class="lbl">Horario</div><div class="val"><?= htmlspecialchars($horario) ?></div>
      </div>
      <div class="i-torneo">
        <div class="lbl">Torneo</div><div class="val"><?= htmlspecialchars($match['torneo_nombre'] ?? '-') ?></div>
      </div>
      <div class="i-cancha">
        <div class="lbl">Cancha</div><div class="val"><?= htmlspecialchars($match['cancha_nombre'] ?? '-') ?></div>
      </div>
      <div class="i-jugadores">
        <div class="lbl"><?= $lblJugadores ?></div>
        <div class="val"><strong><?= $isEquipo?'1':'J1' ?>:</strong> <?= $j1n ?>
        &nbsp;&nbsp;
        <br>
        <strong><?= $isEquipo?'2':'J2' ?>:</strong> <?= $j2n ?></div>
      </div>
    </div>
    ...
    <form method="POST" action="partidosAction.php" id="frmResultado" novalidate>
      ...
      <input
        ...
        aria-describedby="resultadoHint"
        required
      >
...
<script>
/* Marca la pill activa */
(function(){
  const box = document.getElementById('winnerPills');
  if (!box) return;
