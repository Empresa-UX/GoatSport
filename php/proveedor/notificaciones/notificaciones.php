<?php
/* =========================================================================
 * file: php/proveedor/notificaciones/notificaciones.php  (PROVEEDOR)
 * Igual a la vista de Admin pero filtrando por el proveedor logueado.
 * - Rediseño visual (igual reportes)
 * - Filtros: estado / tipo / origen
 * - Pill por origen y tipo
 * - "Visto" con cuadrito (solo marcar como leída; no se desmarca)
...
/* Base WHERE: notificaciones dirigidas al proveedor */
...
/* Badge no leídas */
$sqlCountUnread = "
  SELECT COUNT(*) AS cant FROM notificaciones
...
/* Tipos disponibles (para este proveedor) */
$sqlTipos = "
...
function tipoToLabel(string $tipo): string {
  static $map = [
    ...
    'torneo_partido'        => 'Partido del torneo'
  ];
...
function tipoToClass(string $tipo): string {
  ...
  if ($tipo==='promocion')               return 'notif-tipo-pill notif-tipo-torneo';
  if ($tipo==='evento_especial')         return 'notif-tipo-pill notif-tipo-reporte';
...
<script>
/* Filtros GET con recarga parcial */
(function(){
