...
<div class="section">
  ...
  <?php if ($ok): ?>
    <div style="padding:10px;margin-bottom:15px;border-radius:8px;background:#e1f7e1;color:#2e7d32;"><?= h($ok) ?></div>
  <?php endif; ?>
  <?php if ($err): ?>
    <div style="padding:10px;margin-bottom:15px;border-radius:8px;background:#fdecea;color:#c62828;"><?= h($err) ?></div>
  <?php endif; ?>
  ...
  <div class="form-container" style="max-width:600px;">
    <form action="configuracionAction.php" method="POST" novalidate id="perfilForm">
      ...
      <input type="text" value="<?= h($nombre_club) ?>" readonly>

      <label>Teléfono / WhatsApp:</label>
      <input type="text" value="<?= h($telefono) ?>" readonly>

      <label>Dirección:</label>
      <input type="text" value="<?= h($direccion) ?>" readonly>

      <label>Ciudad:</label>
      <input type="text" value="<?= h($ciudad) ?>" readonly>

      <label>Descripción del club:</label>
      <textarea rows="4" readonly><?= h($descripcion) ?></textarea>
...
(function(){
  ...
  function check(){ btnGuardar.disabled = (snap(form) === initial); }
  ...
  btnCancelar.addEventListener('click', ()=>{
    ...
    else window.location.href = '../home_recepcionista.php'; // fallback
  });
