<?php
/**************************************************************
 * admin/torneos/torneoParticipantes.php
 * BRACKET SOLO LECTURA (ADMIN) — con ganadores reales por ronda
 *************************************************************/
...
/* Si no hay partidos, fallback a participantes */
$participantes = [];
if (!$matchesRows) {
  $ps = $conn->prepare("
...
/* Armar estructura de partidos por id (para propagar ganadores) */
$byId = [];
$maxRonda = 0;
...
/* Propagar ganadores a los siguientes partidos (para mostrar rondas siguientes) */
if ($byId) {
  // ya vienen ordenados por ronda asc; igual lo aseguramos
  ...
  foreach ($matchesRows as $m) {
    ...
    // no sobreescribir si ya hay alguien (por si reprogramaron raro)
    if ($pos === 'j1') {
...
<div class="section">
  ...
  <style>
    :root{ --brand:#0f766e; }
    .btn-add{ background:#1bab9d;color:#fff;border:none;border-radius:8px;padding:8px 12px;cursor:pointer;font-weight:700 }
    .meta-chips{ display:flex; gap:6px; flex-wrap:wrap; }
    .chip{ display:inline-block; padding:4px 10px; border-radius:999px; font-weight:800; font-size:12px; border:1px solid #e2e8f0; background:#fff; color:#475569; }
    ...
    .win-rect{ stroke:#10b981; stroke-width:2; }
    .win-text{ fill:#065f46; }
    .loss-rect{ stroke:#e2e8f0; stroke-width:1; }
  </style>
...
(function(){
  ...
  // Si no hay rounds (no hay partidos): mostramos una “ronda” con seed simple
  if(!rounds.length){
  ...
  // Con partidos: rounds[r-1] = lista de matches de esa ronda
  ...
  for(let r=0;r<ROUNDS;r++){
    ...
    for(let m=0;m<matches;m++){
      ...
      // conexiones a la siguiente ronda
      if (r < ROUNDS-1){
        const nextIdx = Math.floor(m/2);
        const nextX   = M_LEFT + (r+1)*COL_W;
