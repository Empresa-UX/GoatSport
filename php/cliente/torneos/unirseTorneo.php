...
/* Torneo válido + abierto */
$st = $conn->prepare("
  SELECT t.torneo_id, t.nombre, t.estado, t.proveedor_id, COALESCE(prov.nombre,'—') AS club
  FROM torneos t
...
if (!$torneo || $torneo['estado'] !== 'abierto') {
    header("Location: /php/cliente/torneos/torneos.php?err=Torneo no disponible"); exit;
}
...
/* Redirección con alert() en destino */
$destinoSeguro = '/php/cliente/torneos/torneos.php';
if ($return && str_starts_with($return, '/php/cliente/torneos/')) {
    $destinoSeguro = $return;
}
