...
{
  header("Location: reservas.php?ok=" . urlencode($msg));
  exit;
}
function redirect_err(string $msg): void
{
  header("Location: reservas.php?err=" . urlencode($msg));
  exit;
}

/* --------- Helpers promos ---------- */
function day_1_7(string $date): int
{
...
function cancha_capacidad_match(mysqli $conn, int $proveedor_id, int $cancha_id, string $tipo_reserva): void
{
  ...
  if (!$q->fetch()) {
    $q->close();
    redirect_err('Cancha inválida o inactiva.');
  }
  $q->close();
...
if ($action === 'add') {
  ...
  if ($st1->num_rows > 0) {
    $st1->close();
    redirect_err('Conflicto con otra reserva.');
  }
  $st1->close();

  // Conflicto eventos
  $ini_dt = $fecha . ' ' . $hora_inicio;
  ...
  if ($st2->num_rows > 0) {
    $st2->close();
    redirect_err('Conflicto con un evento especial.');
  }
  $st2->close();
  ...
  try {
    ...
    if ($split_costs) {
      // SOLO lo que viene de los inputs split_names[]
      foreach ($split_names as $nm) {
        $nm = trim((string)$nm);
        if ($nm !== '') $integrantes[] = $nm;
      }

      // limpiar duplicados/espacios
      $integrantes = array_values(array_unique(array_filter(array_map('trim', $integrantes))));
    }

    // detalle = nombres separados por coma, o NULL si no hay split
    $detalle = ($split_costs && !empty($integrantes)) ? implode(', ', $integrantes) : null;
    ...
    $stA->bind_param("ssss", $tipo, $origen, $titulo, $msgA);
    if (!$stA->execute()) throw new Exception('No se pudo notificar admins.');
    $stA->close();
    ...
    $stP->bind_param("issss", $proveedor_id, $tipo, $origen, $titulo, $msgP);
    if (!$stP->execute()) throw new Exception('No se pudo notificar proveedor.');
    $stP->close();

    // Notificaciones - partido (si hubo)
    if ($partido_id) {
      $tipo2 = 'partido_nuevo';
      $tit2 = "Partido creado #{$partido_id} (reserva #{$reserva_id})";
      ...
      $stAp->bind_param("ssss", $tipo2, $origen, $tit2, $msg2);
      $stAp->execute();
      $stAp->close();
      $stPp = $conn->prepare("INSERT INTO notificaciones (usuario_id, tipo, origen, titulo, mensaje) VALUES (?,?,?,?,?)");
      $stPp->bind_param("issss", $proveedor_id, $tipo2, $origen, $tit2, $msg2);
      $stPp->execute();
      $stPp->close();
    }
...
if ($action === 'mark_paid') {
  ...
  $stA = $conn->prepare("INSERT INTO notificaciones (usuario_id, tipo, origen, titulo, mensaje) SELECT user_id, ?, ?, ?, ? FROM usuarios WHERE rol='admin'");
  $stA->bind_param("ssss", $tipo, $origen, $titulo, $mensaje);
  $stA->execute();
  $stA->close();

  $stP = $conn->prepare("INSERT INTO notificaciones (usuario_id, tipo, origen, titulo, mensaje) VALUES (?,?,?,?,?)");
  $stP->bind_param("issss", $proveedor_id, $tipo, $origen, $titulo, $mensaje);
  $stP->execute();
  $stP->close();
...
if ($action === 'delete') {
  ...
  if (!$del->execute()) {
    $del->close();
    redirect_err('No se pudo eliminar la reserva.');
  }
  $del->close();
  ...
  try {
    ...
    if ($p) {
      ...
      if ($p['estado'] === 'pendiente') {
        ...
        $upd->bind_param("sdii", $metodo, $precio_final, $creador_id_nuevo, $pid);
        $upd->execute();
        $upd->close();
      }
    }
    ...
    $stC->bind_param("issss", $cl_uid, $tipo, $origen, $titulo, $msgCliente);
    $stC->execute();
    $stC->close();
